...
//   });
  ...
  it('Can import everything automatically that doesnt have required args', async () => {
    const processSpy = vi.spyOn(process, 'cwd');
    processSpy.mockReturnValue('/')
    ...
    const reporter = new MockReporter({
      ...
      promptInitResultSelection: (availableTypes) => {
        expect(availableTypes).toMatchObject(['mock2'])
    ...
    expect(askRequiredParametersSpy).toHaveBeenCalledTimes(0);
    expect(promptInitResultSelectionSpy).toHaveBeenCalledOnce();
    expect(displayFileModifications).toHaveBeenCalledOnce();
    expect(promptConfirmationSpy).toHaveBeenCalledOnce();
  ...
  it('Can automatically import to a partially empty file ([])', async () => {
    const processSpy = vi.spyOn(process, 'cwd');
    processSpy.mockReturnValue('/')

    fs.writeFileSync('/codify.jsonc',
      `[]`,
      { encoding: 'utf-8' });
    ...
    const fileWritten = fs.readFileSync('/codify.jsonc', 'utf8') as string;
    console.log(fileWritten);

    expect(fileWritten).to.eq(
`[
  {
    "type": "mock2",
    "directory": "~/home",
